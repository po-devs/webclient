...
function Pokemon(species) {
	...
	this.checkDetails = function(details) {
	...
	this.addVolatile = function (volatile) {
		...
		if (volatile === 'leechseed') {
			selfP.side.battle.spriteElemsFront[selfP.side.n].append('<img src="libs/ps/fx/energyball.png" style="display:none;position:absolute" />');
...
function Battle(frame, logFrame, noPreload) {
	...
	this.reset = function () {
		...
		self.elem.append('<div class="backdrop" style="background-image:url(libs/ps/fx/' + self.backdropImage + ');display:block;opacity:0"></div>');
		...
		this.updateSidebar = function () {
			...
			for (var i = 0; i < 6; i++) {
				...
				if (i >= selfS.totalPokemon) {
					...
					//pokemonhtml += '<img src="libs/ps/fx/pokeball.png" title="Not revealed" />';
					pokemonhtml += '<span class="pokemonicon" style="'+Tools.getIcon('pokeball')+'" title="Not revealed"></span>';
				//} else if (poke.fainted) {
					//pokemonhtml += '<img src="libs/ps/fx/pokeball.png" style="opacity:0.3;filter:alpha(opacity=30)" title="' + poke.getFullName(true) + '" />';
				} else {
					//pokemonhtml += '<img src="libs/ps/fx/pokeball.png" title="' + poke.getFullName(true) + '" />';
	...
	this.endAction = function () {
		if (self.messageActive) {
			...
			if (!self.fastForward) {
				self.messagebarElem.delay(800 / self.messageSpeed).animate({
					height: 'toggle',
					opacity: 0
				}, 300);
	...
	this.useMove = function (pokemon, move, target, kwargs) {
		...
		if (!self.fastForward) {
			...
			} else if (!kwargs.still) {
				move.anim(self, [pokemon.sprite, target.sprite]);
			}
		}
		if (!kwargs.silent) self.message(pokemon.getName() + ' used <strong>' + move.name + '</strong>!');
	...
	this.runMinor = function (args, kwargs) {
		...
		while (minors.length) {
			...
			case '-clearboost':
				...
				break;
			case '-clearallboost':
				if (self.mySide.active[0]) {
					self.mySide.active[0].boosts = {};
					self.resultAnim(self.mySide.active[0], 'Stats reset', 'neutral', animDelay);
				}
				if (self.yourSide.active[0]) {
					self.yourSide.active[0].boosts = {};
					self.resultAnim(self.yourSide.active[0], 'Stats reset', 'neutral', animDelay);
				}
			...
			case '-singleturn':
				...
				case 'quickguard':
					...
					actions += "Quick Guard protected " + poke.getLowerTeamName() + "!";
					break;
				case 'protect':
			...
			case '-sideend':
				...
				case 'toxicspikes':
					...
					break;
				case 'tailwind':
					actions += "" + side.getTeamName() + " tailwind petered out!";
					break;
				case 'reflect':
	...
	this.getPokemon = function (pokemonid, details) {
		...
		var isOld = false; // if yes, match only pokemon that have been revealed, and can match fainted pokemon
		var isOther = false; // if yes, don't match an active pokemon (for switching)
		var isFaintable = false; // if yes, can match a fainted pokemon
		...
		if (!details) {
			...
			return self.sides[siden].active[slot];
		}
		...
		if (siden !== self.p2.n && !isNew) {
			...
			for (var i = 0; i < self.p1.pokemon.length; i++) {
				...
				if (pokemon.fainted && !isOld && !isFaintable) continue;
				if (pokemon === (self.p1.active[slot]||self.p1.lastPokemon) && isOther) continue;
				if (pokemon.searchid === searchid || (!pokemon.searchid && pokemon.checkDetails(details))) {
					if (!pokemon.searchid)
					{
						pokemon.name = name;
		...
		if (siden !== self.p1.n && !isNew) {
			...
			for (var i = 0; i < self.p2.pokemon.length; i++) {
				...
				if (pokemon.fainted && !isOld && !isFaintable) continue;
				if (pokemon === (self.p2.active[slot]||self.p2.lastPokemon) && isOther) continue;
				if (pokemon.searchid === searchid || (!pokemon.searchid && pokemon.checkDetails(details))) {
					if (!pokemon.searchid)
					{
						pokemon.name = name;
	...
	this.teamPreview = function (start) {
		for (var k = 0; k < 2; k++) {
			...
			if (teamText) {
				self.log('<div class="chat"><strong>' + self.sanitize(self.sides[k].name) + '\'s team:</strong> <em style="color:#445566;display:block;">' + teamText + '</em></div>');
			}
	...
	this.runMajor = function(args, kwargs, preempt) {
		...
		case 'tier':
			...
			self.log('<div><strong>Format:</strong> <em>' + args[1] + '</em></div>');
			break;
		case 'gametype':
			if (args[1] === 'doubles') {
			...
			break;
		case 'variation':
			self.log('<div><strong>Variation:</strong> <em>' + args[1] + '</em></div>');
			break;
		case 'rule':
			self.log('<div><strong>Rule:</strong> <em>' + args[1] + '</em></div>');
			break;
		case 'rated':
			self.rated = true;
		...
		case 'clearpoke':
			self.p1.pokemon = [];
			self.p1.active[0] = null;
			self.p2.pokemon = [];
			self.p2.active[0] = null;
			break;
		case 'poke':
		...
		case 'error':
			args.shift();
			self.message('<strong>Error:</strong> ' + args.join('|'));
		...
		case 'warning':
			args.shift();
			self.message('<strong>Warning:</strong> ' + args.join('|'));
			self.message('Bug? Report it to <a href="http://www.smogon.com/forums/showthread.php?t=3453192">the replay viewer\'s Smogon thread</a>');
		...
		default:
			...
			self.log('<div>Unknown command: ' + args[0] + '</div>');
			break;
		}
	};
		...
		if (str.substr(0,1) === '|') {
			...
			if (self.debug) {
				...
				if (e.stack) {
					var stack = ''+e.stack;
					stack = stack.split("\n").slice(0,2).join("\n");
					self.log('<div class="chat" style="white-space:pre-wrap">' + stack + '</div>', preempt);
				} else {
