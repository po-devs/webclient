...
function BattleRoom(id, elem) {
	...
	this.updateSide = function(sideData, midBattle) {
		...
		for (var i = 0; i < sideData.pokemon.length; i++) {
			...
			var pokemon;
			if (i == 0) {
				pokemon = selfR.battle.getPokemon('canfnt: '+pokemonData.ident, pokemonData.details);
		...
		case 'move':
			{
				...
				for (var i = 0; i < moves.length; i++) {
					...
					if (moveData.disabled) {
						...
						movebuttons += '<button class="type-' + move.type + '" onclick="rooms[\'' + selfR.id + '\'].formUseMove(\'' + moveData.move.replace(/'/g, '\\\'') + '\')"' + tooltipAttrs(moveData.move, 'move') + '>';
						hasMoves = true;
					}
	...
	this.formUseMove = function (move) {
		hideTooltip();
	...
	this.formTeamPreviewSelect = function (pos) {
		pos = parseInt(pos,10);
		hideTooltip();
		if (selfR.teamPreviewHasIllusion) {
			...
			if (selfR.teamPreviewDone < selfR.teamPreviewChoice.length) {
...
function Lobby(id, elem) {
	...
	this.updatePopup = function (data) {
		if (selfR.popupState && !me.popups.length) {
			...
			for (var i = 0; i < me.popups.length - 1; i++) {
				popupListCode += '<h3><button class="closebutton" onclick="rooms[\'' + selfR.id + '\'].popupClose(' + i + ');return false"><i class="icon-remove-sign"></i></button><a onclick="rooms[\'' + selfR.id + '\'].popupFocus(' + i + ');return false">' + sanitize(me.users[me.popups[i]] || me.popups[i]) + '</a></h3>';
			}
			...
			code += popupListCode;
			code += '</div>';
			code += '<h3><button class="closebutton" onclick="rooms[\'' + selfR.id + '\'].popupClose(' + i + ');return false"><i class="icon-remove-sign"></i></button><a>' + sanitize(me.users[me.curPopup] || me.curPopup) + '</a></h3>';
			...
			selfR.popupChatboxElem = selfR.popupElem.find('textarea');
	...
	this.add = function (log) {
		...
		for (var i = 0; i < log.length; i++) {
			...
			if (log[i].name && log[i].message) {
				...
				if (log[i].name.substr(0, 1) !== ' ') group = '<small>' + sanitize(log[i].name.substr(0, 1)) + '</small>';
				if (log[i].pm) {
					var pmuserid = (userid === me.userid ? toUserid(log[i].pm) : userid);
					if (!me.pm[pmuserid]) me.pm[pmuserid] = '';
					var pmcode = '<div class="chat"><strong style="' + color + '">' + group + sanitize(log[i].name.substr(1)) + ':</strong> <em> ' + messageSanitize(message) + '</em></div>';
					...
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">' + group + sanitize(log[i].name.substr(1)) + ':</strong> <em style="color:#007100"><i style="cursor:pointer" onclick="selectTab(\'lobby\');rooms.lobby.popupOpen(\'' + pmuserid + '\')">(Private to ' + sanitize(log[i].pm) + ')</i> ' + messageSanitize(message) + '</em></div>');
				//} else if (log[i].act) {
				//	selfR.chatElem.append('<div class="chat"><strong style="' + color + '">&bull;</strong> <em' + (log[i].name.substr(1) === me.name ? ' class="mine"' : '') + '>' + group + sanitize(log[i].name.substr(1)) + ' <i>' + message + '</i></em></div>');
				} else if (message.substr(0,2) === '//') {
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">' + group + sanitize(log[i].name.substr(1)) + ':</strong> <em' + (log[i].name.substr(1) === me.name ? ' class="mine"' : '') + '>' + messageSanitize(message.substr(1)) + '</em></div>');
				} else if (message.substr(0,4).toLowerCase() === '/me ') {
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">&bull;</strong> <em' + (log[i].name.substr(1) === me.name ? ' class="mine"' : '') + '>' + group + sanitize(log[i].name.substr(1)) + ' <i>' + messageSanitize(message.substr(4)) + '</i></em></div>');
				} else if (message.substr(0,5).toLowerCase() === '/mee ') {
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">&bull;</strong> <em' + (log[i].name.substr(1) === me.name ? ' class="mine"' : '') + '>' + group + sanitize(log[i].name.substr(1)) + '<i>' + messageSanitize(message.substr(5)) + '</i></em></div>');
				} else if (message.substr(0,10).toLowerCase() === '/announce ') {
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">' + group + sanitize(log[i].name.substr(1)) + ':</strong> <em style="padding:1px 4px 2px;color:white;background:#6688AA">' + messageSanitize(message.substr(10)) + '</em></div>');
					...
					selfR.chatElem.append('<div class="chat"><strong style="' + color + '">' + group + sanitize(log[i].name.substr(1)) + ':</strong> <em' + (log[i].name.substr(1) === me.name ? ' class="mine"' : '') + '>' + messageSanitize(message) + '</em></div>');
				}
			} else if (log[i].name && log[i].action === 'battle') {
				var id = log[i].room;
				var matches = id.match(/^battle\-([a-z]*)[0-9]*$/);
	...
	this.command = function (data) {
		if (data.command === 'userdetails') {
			if (!$('#' + selfR.id + '-userdetails-' + data.userid).length) return;
			...
			if (data.ip) {
				roomListCode = '<div class="action-form"><small>IP: <a href="http://www.geoiptool.com/en/?IP=' + data.ip + '" target="iplookup">' + data.ip + '</a></small></div>' + roomListCode;
			}
			$('#' + selfR.id + '-userrooms-' + data.userid).html(roomListCode);
			$('#' + selfR.id + '-userdetails-' + data.userid).html(code);
...
function updateMe() {
	...
	$('#userbar').prepend('[<a href="http://www.smogon.com/forums/showthread.php?t=3469932" target="_blank"><strong>Report bug</strong></a>] ');
